...
public class HVEIP08 {
	...
	public static byte[] enc(CipherParameters publicKey, int... attributes) {
		try {
	...
	public static boolean evaluate(CipherParameters searchKey, byte[] ct) {
		try {
			PredicateOnlyEncryptionScheme engine = new HVEIP08PredicateOnlyEngine();
	...
	public static HVEIP08Parameters genBinaryParam(int n) {
	...
	public static int[][] createNonMatchingVectors(int n) {
		int[][] result = new int[2][n];
		Random random = new Random();
		for (int i = 0; i < n; i++) {
			if (i != 0  && i != 1 && random.nextBoolean()) {// it's a star
	...
	public static void main(String[] args) {
		...
		int n = 5;
		AsymmetricCipherKeyPair keyPair = setup(n);
		...
		//		enc(keyPair.getPublic(), vectors[1])));
